...
std::vector<cv::Scalar> colors(1000, { -1,-1,-1 });
    ...
    if (SUPERPIXELS) {
        for (int i = 0; i < m_labels.size(); i++) {
            for (int j = 0; j < m_labels[i].size(); j++) {
                if (m_labels[i][j] >= 0) {
                    if (colors[m_labels[i][j]] == cv::Scalar(-1, -1, -1))
                    {
                        colors[m_labels[i][j]][0] = rand() % 255;
                        colors[m_labels[i][j]][1] = rand() % 255;
                        colors[m_labels[i][j]][2] = rand() % 255;
                    }
                    //if(m_labels[j][i] % 2 == 0)
                    cv::circle(image_out, cv::Point2f(j, i), 3, colors[m_labels[i][j]], -1);
                }
            }
        }

        for (int i = 0; i < m_poisson_samples.size(); i++) {
            cv::circle(image_out, cv::Point2f(m_poisson_samples[i].y, m_poisson_samples[i].x), 3, cv::Scalar(0, 0, 0), -1);
        }
    }
    else {
